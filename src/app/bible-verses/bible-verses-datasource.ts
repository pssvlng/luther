...
import { Observable, of as observableOf, merge, fromEvent, of } from 'rxjs';
...
export class BibleVersesDataSource extends DataSource<BibleVersesItem> {
  ...
  connect(): Observable<BibleVersesItem[]> {
    ...
    const dataMutations = [
      ...
      fromEvent(this.elementRef.nativeElement.querySelector('mat-paginator'), 'page'),
      //fromEvent(this.paginator, 'page'),
      this.sort ? this.sort.sortChange.asObservable() : of([])
    ];
